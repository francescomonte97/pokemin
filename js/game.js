...
async function initGame() {
  ...
  let selectedGen = [1, 2, 3].includes(Number(localStorage.getItem('poke_selected_gen')))
    ? Number(localStorage.getItem('poke_selected_gen'))
    : 1;
  ...
  if (endlessBtn) {
    if (getHallOfFame().length > 0) {
      ...
      lockOverlay.innerHTML = '<img src="ui/lock.png" style="height:36px;width:auto;image-rendering:pixelated;">';
...
function showGen3ComingSoonModal(modeLabel = 'Normal Mode') {
  ...
  const update = () => {
    ...
    const out = modal.querySelector('#gen3-countdown');
    if (out) out.textContent = `${days}d ${hours}h ${minutes}m ${seconds}s`;
  };

  modal.addEventListener('click', e => { if (e.target === modal) close(); });
  document.body.appendChild(modal);
  modal.querySelector('#gen3-coming-soon-close')?.addEventListener('click', close);
  modal.querySelector('#gen3-coming-soon-ok')?.addEventListener('click', close);
  update();
    ...
    if (panel) {
      ...
      const rows = region.trainers.map((trainer, i) => {
        ...
        return `<div class="${rowClass}" data-species="${speciesAttr}" style="cursor:default;">
          <span class="region-stage-types"><span class="type-badge type-${typeClass}" style="font-size:6px;padding:1px 3px;">${type}</span></span>
...
function showMapScreen() {
  ...
  const BASE = 'https://pokelike.xyz/sprites/badges/';
  let badgeHtml;
  if (state.gen2Mode) {
  ...
  if (state.gen2Mode) {
    ...
    bgUrl = `https://pokelike.xyz/ui/mapsGen2/${state.currentMap + 1}.png`;
  } else {
    bgUrl = `https://pokelike.xyz/ui/mapsNormalMode/map${state.currentMap + 1}.png`;
  }
...
const STAGE_GEN_RANGES = {
  ...
  6: { minGenId: 650, maxGenId: 721 },
  7: { minGenId: 722, maxGenId: 809 },
  8: { minGenId: 810, maxGenId: 905 },
  9: { minGenId: 906, maxGenId: 1025 },
};
function getStageGenRange(stage) {
  return STAGE_GEN_RANGES[stage] || { minGenId: 1, maxGenId: 1025 };
}
...
function getBattleTowerLocations(pokemonId) {
  ...
  const out = [];
  for (let stage = 1; stage <= MAX_ACCESSIBLE_STAGE; stage++) {
      ...
      if (filtered.length > 0) {
        ...
        if (choices.length < 3) {
          ...
          if (lowerIdx < getEncounterMapIndex()) {
            const genRange = getStageGenRange(endlessState.stageNumber);
            const lowerPool = await getCatchChoices(lowerIdx, 18, genRange.maxGenId, false, genRange.minGenId, state.isEndlessMode);
...
function checkDexAchievements() {
  ...
  if (isShinyGenDexComplete(1, 1025)) {
  ...
  const genRanges = [
    ...
    ['pokedex_gen7', 722, 809],
    ['pokedex_gen8', 810, 905],
    ['pokedex_gen9', 906, 1025],
  ];
...
async function doTrainerNode(node) {
  ...
  let speciesList;
  const activePool = (state.isEndlessMode && endlessState.stageNumber >= 6)
    ? null
...
function showBadgeScreen(leader) {
  ...
  if (badgeImg) {
    if (state.gen2Mode) {
      ...
      badgeImg.src = `https://pokelike.xyz/sprites/badges/${state.badges + 8}.png`;
    } else {
      badgeImg.src = `https://pokelike.xyz/sprites/badges/${state.badges}.png`;
    }
  }
...
const STAGE_META = [
  ...
  { label: 'Kalos',  gens: 'Gen 6', color: '#d878c8' },
  { label: 'Alola',  gens: 'Gen 7', color: '#f0b848' },
  { label: 'Galar',  gens: 'Gen 8', color: '#48a8d8' },
  { label: 'Paldea', gens: 'Gen 9', color: '#c85050' },
];

const BATTLE_TOWER_MAP_BACKGROUNDS = [
  'https://pokelike.xyz/ui/mapsBattleTower/normal.png',
  'https://pokelike.xyz/ui/mapsBattleTower/fire.png',
  'https://pokelike.xyz/ui/mapsBattleTower/water.png',
  'https://pokelike.xyz/ui/mapsBattleTower/grass.png',
  'https://pokelike.xyz/ui/mapsBattleTower/electric.png',
  'https://pokelike.xyz/ui/mapsBattleTower/dragon.png',
  'https://pokelike.xyz/ui/mapsBattleTower/dark.png',
  'https://pokelike.xyz/ui/mapsBattleTower/steel.png',
  'https://pokelike.xyz/ui/mapsBattleTower/ghost.png',
  'https://pokelike.xyz/ui/mapsBattleTower/psychic.png',
  'https://pokelike.xyz/ui/mapsBattleTower/fighting.png',
  'https://pokelike.xyz/ui/mapsBattleTower/ground.png',
  'https://pokelike.xyz/ui/mapsBattleTower/rock.png',
  'https://pokelike.xyz/ui/mapsBattleTower/ice.png',
  'https://pokelike.xyz/ui/mapsBattleTower/poison.png',
  'https://pokelike.xyz/ui/mapsBattleTower/flying.png',
  'https://pokelike.xyz/ui/mapsBattleTower/bug.png',
  'https://pokelike.xyz/ui/mapsBattleTower/finalBoss.png',
];
const STAGE_REGION_FALLBACK_BG = BATTLE_TOWER_MAP_BACKGROUNDS[0];
const STAGE_REGION_BG = [
  null,
  { src: 'https://pokelike.xyz/ui/regions/HGSS_Kanto.jpg', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'https://pokelike.xyz/ui/regions/Johtoart.jpg', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'https://pokelike.xyz/ui/regions/ORAS_Hoenn_Map.jpg', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'https://pokelike.xyz/ui/regions/Pt_Artwork_Sinnoh-Karte_(mit_Zerrwelt).jpg', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'https://pokelike.xyz/ui/regions/Einall_S2W2.png', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'ui/regions/kalos.jpg', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'ui/regions/alola.png', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'ui/regions/galar.png', fallback: STAGE_REGION_FALLBACK_BG },
  { src: 'ui/regions/paldea.png', fallback: STAGE_REGION_FALLBACK_BG },
];

const _stageRegionBgPreload = new Map();

function getStageRegionBgEntry(stage) {
  const entry = STAGE_REGION_BG[stage];
  return typeof entry === 'string' ? { src: entry, fallback: STAGE_REGION_FALLBACK_BG } : entry;
}

function buildStageRegionBgStyle(stage) {
  const entry = getStageRegionBgEntry(stage);
  if (!entry?.src) return `background:linear-gradient(135deg,#1a0a3e,#3a0a6e);`;
  const fallback = entry.fallback ? `,url('${entry.fallback}')` : '';
  return `background-image:url('${entry.src}')${fallback};background-size:cover;background-position:center;`;
}

function getBattleTowerMapBg(type, isFinalBoss = false) {
  if (isFinalBoss) return 'https://pokelike.xyz/ui/mapsBattleTower/finalBoss.png';
  const normalized = String(type || 'normal').toLowerCase();
  const available = new Set([
    'normal', 'fire', 'water', 'grass', 'electric', 'dragon',
    'dark', 'steel', 'ghost', 'psychic', 'fighting', 'ground',
    'rock', 'ice', 'poison', 'flying', 'bug',
  ]);
  const key = available.has(normalized) ? normalized : 'normal';
  return `https://pokelike.xyz/ui/mapsBattleTower/${key}.png`;
}

function preloadStageRegionBackgrounds() {
  if (typeof document === 'undefined') return;
  const urls = [];
  for (let s = 1; s <= MAX_ACCESSIBLE_STAGE; s++) {
    const entry = getStageRegionBgEntry(s);
    if (entry?.fallback) urls.push(entry.fallback);
    if (entry?.src) urls.push(entry.src);
  }
  for (const url of [...urls, ...BATTLE_TOWER_MAP_BACKGROUNDS]) {
    if (!url || _stageRegionBgPreload.has(url)) continue;
    ...
    img.src = url;
    _stageRegionBgPreload.set(url, img);
  }
}
...
function showEndlessStageSelect() {
  ...
  if (!list) return;
  list.innerHTML = '';
  const maxShow = MAX_ACCESSIBLE_STAGE;
  for (let s = 1; s <= maxShow; s++) {
    ...
    const bgStyle = buildStageRegionBgStyle(s);
    btn.style.cssText = `width:200px;font-family:'Press Start 2P',monospace;${isLocked ? `opacity:0.45;cursor:not-allowed;${bgStyle}` : `${bgStyle}${borderColor ? `border-color:${borderColor};box-shadow:0 0 6px ${borderColor}55;` : ''}`}`;
    if (isLocked) {
      btn.innerHTML = `<div style="font-family:'Press Start 2P',monospace;background:rgba(0,0,0,0.55);padding:4px 8px;border-radius:4px;color:#fff;"><div style="font-size:8px;line-height:1.45;">🔒 ${getStageName(s)}</div><div style="font-size:5px;line-height:1.35;opacity:0.85;margin-top:2px;">${meta?.gens || 'All Gens'}</div></div>`;
    } else if (meta) {
      btn.innerHTML = `<div style="font-family:'Press Start 2P',monospace;background:rgba(0,0,0,0.5);padding:4px 8px;border-radius:4px;color:#fff;"><div style="font-size:8px;line-height:1.45;">▶ ${meta.label}</div><div style="font-size:5px;line-height:1.35;opacity:0.85;margin-top:2px;">${meta.gens}</div></div>`;
    } else {
      btn.innerHTML = `<div style="font-family:'Press Start 2P',monospace;background:rgba(0,0,0,0.5);padding:4px 8px;border-radius:4px;color:#fff;"><div style="font-size:8px;line-height:1.45;">▶ ${getStageName(s)}</div><div style="font-size:5px;line-height:1.35;opacity:0.85;margin-top:2px;">All Gens</div></div>`;
    }
...
function startEndlessMap() {
  ...
  endlessState.currentMapBg = _isFinalBoss
    ? getBattleTowerMapBg(_btType, true)
    : getBattleTowerMapBg(_btType);
...
function showEndlessMapScreen() {
  ...
  const mapContainer = document.getElementById('map-container');
  const bg = endlessState.currentMapBg || 'https://pokelike.xyz/ui/mapsNormalMode/map1.png';
...
function advanceEndless() {
  ...
  if (endlessState.mapIndexInRegion >= 3) {
    ...
    if (endlessState.regionNumber > 3) {
      ...
      [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach(threshold => {
