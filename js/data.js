...
const ITEM_POOL = [
  ...
  { id: 'loaded_dice',        name: 'Loaded Dice',        desc: 'Start of each battle: 37% chance for +2 to ATK/DEF/Sp.Atk/Sp.Def/Speed, else -1', icon: '🎲', iconUrl: 'https://pokelike.xyz/sprites/items/loaded_dice.png', gen2Only: true },
];
...
// Keyed by numeric id. Avoids per-pokemon PokeAPI fetches for the covered species.
...
function loadStaticPokedex() {
  ...
  _staticPokedexPromise = fetch('data/pokedex.json')
    ...
    .then(d => {
      _staticPokedex = d || {};
      if (typeof _expandedBstBuckets !== 'undefined') _expandedBstBuckets = null;
      return _staticPokedex;
    })
...
// is the primary source. If it hasn't
...
// Legendary Pokemon — excluded from wild/catch pools, available only via Legendary node
const LEGENDARY_IDS = [
...
// Catchable Pokemon by BST bucket — Gen 1-5 hand-tuned; later Battle Tower
// stages add Gen 6-8 from the static pokedex using the same BST thresholds.
...
const EXTENDED_BST_MIN_ID = 650;
const EXTENDED_BST_MAX_ID = 1025;
let _expandedBstBuckets = null;

function getExpandedBstBuckets() {
  if (_expandedBstBuckets) return _expandedBstBuckets;
  const buckets = Object.fromEntries(
    Object.entries(GEN1_BST_APPROX).map(([k, v]) => [k, [...v]])
  );
  const dex = _staticPokedex || {};
  for (let id = EXTENDED_BST_MIN_ID; id <= EXTENDED_BST_MAX_ID; id++) {
    if (LEGENDARY_ID_SET.has(id)) continue;
    const stats = dex[id]?.baseStats;
    if (!stats) continue;
    const bst = Object.values(stats).reduce((a, b) => a + b, 0);
    const bucket = bst >= 530 ? 'veryHigh'
      : bst >= 460 ? 'high'
      : bst >= 400 ? 'midHigh'
      : bst >= 340 ? 'mid'
      : bst >= 280 ? 'midLow'
  ...
  _expandedBstBuckets = buckets;
  return buckets;
}

const ALL_CATCHABLE_IDS = new Set([
  ...Array.from({ length: 1025 }, (_, i) => i + 1).filter(id => !LEGENDARY_ID_SET.has(id)),
]);
...
// maxGenId restricts to IDs <= that number (151 = Gen 1 only, 1025 = through Gen 9).
...
function getTrainerImgHtml(trainerName) {
  // Local sprite path (e.g. "https://pokelike.xyz/sprites/hiker.png") — use directly
...
const EVOLUTIONS = {
  ...
  // Gen 6
  650:{ into: 651, level: 16, name: 'Quilladin' },
  651:{ into: 652, level: 36, name: 'Chesnaught' },
  653:{ into: 654, level: 16, name: 'Braixen' },
  654:{ into: 655, level: 36, name: 'Delphox' },
  656:{ into: 657, level: 16, name: 'Frogadier' },
  657:{ into: 658, level: 36, name: 'Greninja' },
  659:{ into: 660, level: 20, name: 'Diggersby' },
  661:{ into: 662, level: 17, name: 'Fletchinder' },
  662:{ into: 663, level: 35, name: 'Talonflame' },
  664:{ into: 665, level: 9,  name: 'Spewpa' },
  665:{ into: 666, level: 12, name: 'Vivillon' },
  667:{ into: 668, level: 35, name: 'Pyroar' },
  669:{ into: 670, level: 19, name: 'Floette' },
  670:{ into: 671, level: 36, name: 'Florges' },
  672:{ into: 673, level: 32, name: 'Gogoat' },
  674:{ into: 675, level: 32, name: 'Pangoro' },
  677:{ into: 678, level: 25, name: 'Meowstic' },
  679:{ into: 680, level: 35, name: 'Doublade' },
  680:{ into: 681, level: 36, name: 'Aegislash' },
  682:{ into: 683, level: 40, name: 'Aromatisse' },
  684:{ into: 685, level: 40, name: 'Slurpuff' },
  686:{ into: 687, level: 30, name: 'Malamar' },
  688:{ into: 689, level: 39, name: 'Barbaracle' },
  690:{ into: 691, level: 48, name: 'Dragalge' },
  692:{ into: 693, level: 37, name: 'Clawitzer' },
  694:{ into: 695, level: 36, name: 'Heliolisk' },
  696:{ into: 697, level: 39, name: 'Tyrantrum' },
  698:{ into: 699, level: 39, name: 'Aurorus' },
  704:{ into: 705, level: 40, name: 'Sliggoo' },
  705:{ into: 706, level: 50, name: 'Goodra' },
  708:{ into: 709, level: 40, name: 'Trevenant' },
  710:{ into: 711, level: 40, name: 'Gourgeist' },
  712:{ into: 713, level: 37, name: 'Avalugg' },
  714:{ into: 715, level: 48, name: 'Noivern' },
  // Gen 7
  722:{ into: 723, level: 17, name: 'Dartrix' },
  723:{ into: 724, level: 34, name: 'Decidueye' },
  725:{ into: 726, level: 17, name: 'Torracat' },
  726:{ into: 727, level: 34, name: 'Incineroar' },
  728:{ into: 729, level: 17, name: 'Brionne' },
  729:{ into: 730, level: 34, name: 'Primarina' },
  731:{ into: 732, level: 14, name: 'Trumbeak' },
  732:{ into: 733, level: 28, name: 'Toucannon' },
  734:{ into: 735, level: 20, name: 'Gumshoos' },
  736:{ into: 737, level: 20, name: 'Charjabug' },
  737:{ into: 738, level: 40, name: 'Vikavolt' },
  739:{ into: 740, level: 40, name: 'Crabominable' },
  742:{ into: 743, level: 25, name: 'Ribombee' },
  744:{ into: 745, level: 25, name: 'Lycanroc' },
  747:{ into: 748, level: 38, name: 'Toxapex' },
  749:{ into: 750, level: 30, name: 'Mudsdale' },
  751:{ into: 752, level: 22, name: 'Araquanid' },
  753:{ into: 754, level: 34, name: 'Lurantis' },
  755:{ into: 756, level: 24, name: 'Shiinotic' },
  757:{ into: 758, level: 33, name: 'Salazzle' },
  759:{ into: 760, level: 27, name: 'Bewear' },
  761:{ into: 762, level: 18, name: 'Steenee' },
  762:{ into: 763, level: 30, name: 'Tsareena' },
  767:{ into: 768, level: 30, name: 'Golisopod' },
  769:{ into: 770, level: 42, name: 'Palossand' },
  772:{ into: 773, level: 30, name: 'Silvally' },
  782:{ into: 783, level: 35, name: 'Hakamo-o' },
  783:{ into: 784, level: 45, name: 'Kommo-o' },
  789:{ into: 790, level: 43, name: 'Cosmoem' },
  803:{ into: 804, level: 30, name: 'Naganadel' },
  // Gen 8
  810:{ into: 811, level: 16, name: 'Thwackey' },
  811:{ into: 812, level: 35, name: 'Rillaboom' },
  813:{ into: 814, level: 16, name: 'Raboot' },
  814:{ into: 815, level: 35, name: 'Cinderace' },
  816:{ into: 817, level: 16, name: 'Drizzile' },
  817:{ into: 818, level: 35, name: 'Inteleon' },
  819:{ into: 820, level: 24, name: 'Greedent' },
  821:{ into: 822, level: 18, name: 'Corvisquire' },
  822:{ into: 823, level: 38, name: 'Corviknight' },
  824:{ into: 825, level: 10, name: 'Dottler' },
  825:{ into: 826, level: 30, name: 'Orbeetle' },
  827:{ into: 828, level: 18, name: 'Thievul' },
  829:{ into: 830, level: 20, name: 'Eldegoss' },
  831:{ into: 832, level: 24, name: 'Dubwool' },
  833:{ into: 834, level: 22, name: 'Drednaw' },
  835:{ into: 836, level: 25, name: 'Boltund' },
  837:{ into: 838, level: 18, name: 'Carkol' },
  838:{ into: 839, level: 34, name: 'Coalossal' },
  843:{ into: 844, level: 36, name: 'Sandaconda' },
  846:{ into: 847, level: 26, name: 'Barraskewda' },
  848:{ into: 849, level: 30, name: 'Toxtricity' },
  850:{ into: 851, level: 28, name: 'Centiskorch' },
  852:{ into: 853, level: 30, name: 'Grapploct' },
  854:{ into: 855, level: 36, name: 'Polteageist' },
  856:{ into: 857, level: 32, name: 'Hattrem' },
  857:{ into: 858, level: 42, name: 'Hatterene' },
  859:{ into: 860, level: 32, name: 'Morgrem' },
  860:{ into: 861, level: 42, name: 'Grimmsnarl' },
  868:{ into: 869, level: 36, name: 'Alcremie' },
  872:{ into: 873, level: 30, name: 'Frosmoth' },
  878:{ into: 879, level: 34, name: 'Copperajah' },
  885:{ into: 886, level: 50, name: 'Drakloak' },
  886:{ into: 887, level: 60, name: 'Dragapult' },
  891:{ into: 892, level: 36, name: 'Urshifu' },
  // Gen 9
  906:{ into: 907, level: 16, name: 'Floragato' },
  907:{ into: 908, level: 36, name: 'Meowscarada' },
  909:{ into: 910, level: 16, name: 'Crocalor' },
  910:{ into: 911, level: 36, name: 'Skeledirge' },
  912:{ into: 913, level: 16, name: 'Quaxwell' },
  913:{ into: 914, level: 36, name: 'Quaquaval' },
  915:{ into: 916, level: 18, name: 'Oinkologne' },
  917:{ into: 918, level: 15, name: 'Spidops' },
  919:{ into: 920, level: 24, name: 'Lokix' },
  921:{ into: 922, level: 18, name: 'Pawmo' },
  922:{ into: 923, level: 36, name: 'Pawmot' },
  924:{ into: 925, level: 25, name: 'Maushold' },
  926:{ into: 927, level: 26, name: 'Dachsbun' },
  928:{ into: 929, level: 25, name: 'Dolliv' },
  929:{ into: 930, level: 35, name: 'Arboliva' },
  932:{ into: 933, level: 24, name: 'Naclstack' },
  933:{ into: 934, level: 38, name: 'Garganacl' },
  938:{ into: 939, level: 26, name: 'Bellibolt' },
  940:{ into: 941, level: 25, name: 'Kilowattrel' },
  942:{ into: 943, level: 30, name: 'Mabosstiff' },
  944:{ into: 945, level: 28, name: 'Grafaiai' },
  946:{ into: 947, level: 30, name: 'Brambleghast' },
  948:{ into: 949, level: 30, name: 'Toedscruel' },
  951:{ into: 952, level: 30, name: 'Scovillain' },
  953:{ into: 954, level: 24, name: 'Rabsca' },
  955:{ into: 956, level: 35, name: 'Espathra' },
  957:{ into: 958, level: 24, name: 'Tinkatuff' },
  958:{ into: 959, level: 38, name: 'Tinkaton' },
  960:{ into: 961, level: 26, name: 'Wugtrio' },
  963:{ into: 964, level: 38, name: 'Palafin' },
  965:{ into: 966, level: 32, name: 'Revavroom' },
  969:{ into: 970, level: 35, name: 'Glimmora' },
  971:{ into: 972, level: 30, name: 'Houndstone' },
  974:{ into: 975, level: 34, name: 'Cetitan' },
  996:{ into: 997, level: 35, name: 'Arctibax' },
  997:{ into: 998, level: 54, name: 'Baxcalibur' },
  1012:{ into: 1013, level: 36, name: 'Sinistcha' },
};
...
const BRANCHING_EVOLUTIONS = {
  133: [ // Eevee
    ...
    { into: 700, level: 30, name: 'Sylveon',   types: ['Fairy'] },
  ],
  44: [ // Gloom
  ...
  790: [ // Cosmoem
    { into: 791, level: 53, name: 'Solgaleo', types: ['Psychic', 'Steel'] },
    { into: 792, level: 53, name: 'Lunala',   types: ['Psychic', 'Ghost'] },
  ],
  840: [ // Applin
    { into: 841, level: 36, name: 'Flapple',  types: ['Grass', 'Dragon'] },
    { into: 842, level: 36, name: 'Appletun', types: ['Grass', 'Dragon'] },
  ],
  935: [ // Charcadet
    { into: 936, level: 36, name: 'Armarouge', types: ['Fire', 'Psychic'] },
    { into: 937, level: 36, name: 'Ceruledge', types: ['Fire', 'Ghost'] },
  ],
  999: [ // Gimmighoul
    { into: 1000, level: 36, name: 'Gholdengo', types: ['Steel', 'Ghost'] },
  ],
};
...
const ACHIEVEMENTS = [
  ...
  { id: 'endless_stage_6',  name: 'Kalos Champion',  desc: 'Defeat Diantha and clear Stage 6 of Battle Tower',        icon: '🏆', img: 'item:fairy-plate', category: 'tower' },
  { id: 'endless_stage_7',  name: 'Alola Champion',  desc: 'Defeat Professor Kukui and clear Stage 7 of Battle Tower', icon: '🌴', img: 'item:kommonium-z', category: 'tower' },
  { id: 'endless_stage_8',  name: 'Galar Champion',  desc: 'Defeat Leon and clear Stage 8 of Battle Tower',            icon: '🛡️', img: 'item:rusted-sword', category: 'tower' },
  { id: 'endless_stage_9',  name: 'Paldea Champion', desc: 'Clear Stage 9 of Battle Tower',                            icon: '🍊', img: 'item:master-ball', category: 'tower' },
  ...
  { id: 'starters_stage_6', name: "Sycamore's Challenge", desc: 'Win a Stage 6 run starting with Chespin, Fennekin, or Froakie',      icon: '🌸', img: 'item:pixie-plate', category: 'tower' },
  { id: 'starters_stage_7', name: "Kukui's Challenge",    desc: 'Win a Stage 7 run starting with Rowlet, Litten, or Popplio',         icon: '🌺', img: 'item:fire-stone', category: 'tower' },
  { id: 'starters_stage_8', name: "Magnolia's Challenge", desc: 'Win a Stage 8 run starting with Grookey, Scorbunny, or Sobble',      icon: '🎖️', img: 'item:rare-candy', category: 'tower' },
  { id: 'starters_stage_9', name: "Clavell's Challenge",  desc: 'Win a Stage 9 run starting with Sprigatito, Fuecoco, or Quaxly',     icon: '🍇', img: 'item:ultra-ball', category: 'tower' },
  ...
  { id: 'pokedex_gen6', name: 'Kalos Completionist',  desc: 'Catch all Gen 6 Pokémon across any number of runs', icon: '📓', img: 'item:pixie-plate', category: 'general' },
  { id: 'pokedex_gen7', name: 'Alola Completionist',  desc: 'Catch all Gen 7 Pokémon across any number of runs', icon: '📔', img: 'item:ultra-ball', category: 'general' },
  { id: 'pokedex_gen8', name: 'Galar Completionist',  desc: 'Catch all Gen 8 Pokémon across any number of runs', icon: '📒', img: 'item:great-ball', category: 'general' },
  { id: 'pokedex_gen9', name: 'Paldea Completionist', desc: 'Catch all Gen 9 Pokémon across any number of runs', icon: '📚', img: 'item:master-ball', category: 'general' },
...
function achievementIconHtml(a) {
  ...
  const path = m[1] === 'pkmn'  ? `${pokeApi}pokemon/${m[2]}.png`
             ...
             :                    `https://pokelike.xyz/sprites/badges/${m[2]}.png`;
