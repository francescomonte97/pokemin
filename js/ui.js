...
async function openPokedexModal(initialTab = 'normal') {
  ...
  function buildNormalGrid() {
    ...
    const caughtCount = Array.from({length: DEX_MAX_ID}, (_, i) => i + 1).filter(id => _isDexCaught(dex[id])).length;
    ...
    const towerStageFor = (typeof getBattleTowerLocations === 'function')
      ...
      : () => null;
    const grid = Array.from({ length: DEX_MAX_ID }, (_, i) => {
      const id = i + 1;
  ...
  function buildShinyGrid() {
    ...
    const count = Array.from({length: DEX_MAX_ID}, (_, i) => i + 1).filter(id => dex[id]).length;
  ...
  function switchTab(tab) {
    ...
    const allIds = Array.from({ length: DEX_MAX_ID }, (_, i) => i + 1);
...
function getExtraDexRegion(speciesId) {
  if (speciesId >= 650 && speciesId <= 721) return { stage: 6, name: 'Kalos' };
  if (speciesId >= 722 && speciesId <= 809) return { stage: 7, name: 'Alola' };
  if (speciesId >= 810 && speciesId <= 905) return { stage: 8, name: 'Galar' };
  if (speciesId >= 906 && speciesId <= 1025) return { stage: 9, name: 'Paldea' };
  return null;
}

function getExtraDexTowerLabels(speciesId) {
  const region = getExtraDexRegion(speciesId);
  const entry = typeof getStaticPokedexEntry === 'function' ? getStaticPokedexEntry(speciesId) : null;
  const stats = entry?.baseStats || {};
  const bst = Object.values(stats).reduce((a, b) => a + Number(b || 0), 0);
  let floors;
  if (bst < 330) floors = ['R1M1', 'R1M2', 'R1M3'];
  else if (bst < 410) floors = ['R1M2', 'R1M3', 'R2M1'];
  else if (bst < 480) floors = ['R2M1', 'R2M2', 'R2M3'];
  else if (bst < 540) floors = ['R2M3', 'R3M1', 'R3M2'];
  else floors = ['R3M1', 'R3M2', 'R3M3'];
  return region ? floors.map(f => `${region.name} ${f}`) : [];
}

function _speciesIdFromPokeApiUrl(url) {
  const m = String(url || '').match(/\/pokemon-species\/(\d+)\/?$/);
  return m ? Number(m[1]) : null;
}

async function fetchPokeApiEvolutionChain(speciesId) {
  ...
  if (!chainRes.ok) throw new Error(`Evolution HTTP ${chainRes.status}`);
  return (await chainRes.json()).chain || null;
}

function _evolutionLevelText(details) {
...
function openDexDetailModal(speciesId, name, spriteUrl, shinySpriteUrl, types) {
  ...
  const towerFloors = extraRegion
    ? getExtraDexTowerLabels(speciesId)
    : (typeof getBattleTowerLocations === 'function')
  ...
  (async () => {
    if (speciesId > 649) {
      try {
        const apiChain = await fetchPokeApiEvolutionChain(speciesId);
        const evoEl = document.getElementById('dex-detail-evo');
        if (!evoEl || !apiChain) return;
        evoEl.innerHTML = '';

        async function renderApiNode(node) {
          const id = _speciesIdFromPokeApiUrl(node?.species?.url);
          if (!id) return null;
          ...
          img.src = poke?.spriteUrl || `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png`;
          img.alt = poke?.name || '';
          const nameEl = document.createElement('div');
          nameEl.className = 'dex-evo-name';
          nameEl.textContent = poke?.name || getSpeciesName(id);
          nodeEl.appendChild(img);
          nodeEl.appendChild(nameEl);
          wrap.appendChild(nodeEl);

          const evos = node.evolves_to || [];
          if (evos.length > 0) {
            ...
            levelEl.textContent = _evolutionLevelText(evos[0].evolution_details);
            wrap.appendChild(document.createElement('div')).className = 'dex-evo-arrow-wrap';
            wrap.lastChild.appendChild(levelEl);
            wrap.lastChild.appendChild(arrow);
            const nextWrap = document.createElement('div');
            nextWrap.style.display = 'flex';
            nextWrap.style.gap = '8px';
            nextWrap.style.flexWrap = 'wrap';
            for (const child of evos) {
              const rendered = await renderApiNode(child);
              if (rendered) nextWrap.appendChild(rendered);
            }
        ...
        const rendered = await renderApiNode(apiChain);
        if (rendered && document.getElementById('dex-detail-evo')) evoEl.appendChild(rendered);
      } catch {
        const evoEl = document.getElementById('dex-detail-evo');
